...
export default function UserDashboard() {
  ...
  return (
    <div className={`user-dashboard-redesign relative overflow-x-hidden bg-[#FFFFF0] text-black selection:bg-[#2775CA]/20 selection:text-black md:h-[100dvh] md:overflow-hidden ${
          ...
          <div className={`col-span-1 ${
              ...
              <section className="space-y-5 max-w-lg pb-6 lg:pb-0">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                  ...
                  <div ref={batchFormRef} className="border border-black/10 dark:border-white/15 bg-white/80 dark:bg-white/[0.04] rounded-3xl p-5 sm:p-8 space-y-6 shadow-sm text-black dark:text-white">
                    ...
                    {/* Capped client-side against the same constant the send route enforces. */}
                    <button
                      type="button"
                      onClick={() => setBatchRows((rows) => (
                        rows.length >= MAX_BATCH_RECIPIENTS ? rows : [...rows, { address: "", amount: "" }]
                      ))}
                      disabled={batchRows.length >= MAX_BATCH_RECIPIENTS || batchSendLoading}
            ...
            {activeTab === "referrals" && (
              <section className="space-y-6 pb-20 max-w-2xl">
                ...
                {referralQrOpen && (
                  <div
                    className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 p-3 sm:p-4 backdrop-blur-md animate-in fade-in duration-200"
                    role="dialog"
                    aria-modal="true"
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="flex flex-col items-center justify-center rounded-2xl border border-black/10 dark:border-white/10 bg-[#f8fafc] dark:bg-white/[0.03] p-4 sm:p-5 shadow-inner">
                        <div id="referral-qr-canvas-wrap" className="rounded-2xl bg-white p-2.5 sm:p-3 shadow-md border border-black/10 flex items-center justify-center w-[176px] sm:w-[200px] h-[176px] sm:h-[200px]">
                          <QRCode
                            value={referralLink}
                            size={172}
                            ecLevel="H"
      ...
      {/* The shared component, the same one the merchant dashboard and payroll use. This page used to
          render a private inline copy of the same name, which silently shadowed the import and left
          the Deposit button opening the old modal with no chain picker and no fees. */}
      <DepositModal
        isOpen={receiveOpen}
