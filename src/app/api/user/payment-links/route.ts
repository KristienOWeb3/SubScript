...
export async function POST(request: Request) {
    try {
        ...
        const { amountUsdc, title, description, expiresInHours, billingType, isRecurring } = sanitizeInput(body);

        if (billingType === "RECURRING" || isRecurring === true) {
            return NextResponse.json(
                { error: "Recurring payment links are strictly reserved for Enterprise merchants. User payment links are one-time USDC transfers." },
                { status: 400 }
            );
        }
        ...
        const checkoutUrl = buildCheckoutUrl(paymentRequest.paymentLinkId, origin);
