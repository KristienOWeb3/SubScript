...
export async function GET(request: Request) {
    try {
        ...
        const [merchantRes, subRes] = await Promise.all([
            ...
            supabase
                .from("subscriptions")
                .select("subscription_id, cancel_at_period_end, next_billing_date")
        ...
        if (merchantRes.error) {
            ...
            return NextResponse.json({ tier: 0, subscriptionId: null, cancelAtPeriodEnd: false, nextBillingDate: null }, { status: 200 });
        }
        ...
        return NextResponse.json({ tier, subscriptionId, cancelAtPeriodEnd, nextBillingDate }, { status: 200 });
    } catch (error) {
