    ...
    components: {
        ...
        schemas: {
            ...
            Subscription: {
                type: "object",
                properties: {
                    id: { type: "string", description: "sub_<checkout uuid> before activation, sub_<number> for an on-chain read. Either form is accepted by GET /api/v1/subscriptions/{id}." },
                    object: { type: "string", const: "subscription" },
                    status: {
                        type: "string",
                        enum: ["incomplete", "active", "past_due", "canceled", "expired", "inactive"],
                        description: "Derived from the billing record, not the checkout: a subscription canceled after activation reports `canceled`. `expired` marks a checkout abandoned for 24h. `inactive` only appears on on-chain reads whose PSA authorization is revoked.",
                    },
                    merchantAddress: { type: "string" },
                    subscriptionId: { type: ["string", "null"], description: "On-chain PSA id, null until the authorization settles. Required by DELETE for an active subscription." },
                    subscriber: { type: ["string", "null"], description: "Wallet that authorized the subscription. Null only while the checkout is unaccepted." },
                    ...
                    currentPeriodEnd: { type: ["string", "null"], format: "date-time", description: "End of the paid period — when access lapses without a renewal. Null while incomplete or expired. Same value the merchant dashboard renders; do not recompute it from createdAt + intervalSeconds." },
                    currentPeriodEndTimestamp: { type: ["integer", "null"], description: "Unix seconds form of currentPeriodEnd." },
                    nextPaymentDate: { type: ["string", "null"], format: "date-time", description: "Alias of currentPeriodEnd." },
                    merchantCustomerId: { type: ["string", "null"], description: "Merchant-owned customer/account binding, returned on every read so the webhook is not the only mapping." },
                    externalReference: { type: ["string", "null"], description: "Backwards-compatible alias for merchantCustomerId." },
                    planId: { type: ["string", "null"] },
                    checkoutUrl: { type: "string", format: "uri" },
                    expiresAt: { type: ["string", "null"], format: "date-time", description: "When an unaccepted checkout stops being payable. Accepting an expired checkout returns 410 CHECKOUT_EXPIRED." },
    ...
    paths: {
        ...
        "/api/v1/subscriptions": {
            ...
            get: {
                ...
                description: "No params lists this merchant's subscription checkout sessions, each enriched from the billing record — `status`, `currentPeriodEnd`, `subscriber`, `subscriptionId` and `externalReference` all come back. `?id=` reads one subscription and accepts either id form. `?subscriber=0x…` lists a subscriber's on-chain subscriptions. Bounded to the 100 most recent; `status` and `externalReference` filter within that window.",
                parameters: [
                    { name: "id", in: "query", required: false, schema: { type: "string" }, description: "sub_<uuid> or sub_<number>. Prefer GET /api/v1/subscriptions/{id}." },
                    { name: "subscriber", in: "query", required: false, schema: { type: "string" } },
                    {
                        ...
                        required: false,
                        schema: { type: "string" },
                        description: "Comma-separated: incomplete, active, past_due, canceled, expired. An unknown value is rejected rather than ignored.",
                    },
                    {
                        ...
                        required: false,
                        schema: { type: "string" },
                        description: "Exact match on your own customer/account binding. `merchantCustomerId` is accepted as an alias.",
                    },
                ],
                responses: {
                    ...
                    "400": { description: "Unknown status value, or an id in neither supported form", content: { "application/json": { schema: { $ref: "#/components/schemas/Error" } } } },
                },
            },
            delete: {
        ...
        "/api/v1/subscriptions/{id}": {
            get: {
                ...
                description: "Single-intent read. Accepts either id the API hands out: `sub_<uuid>` for a checkout session (what the list returns) or `sub_<number>` for an on-chain subscription. Retrieving one subscription no longer means listing all of them and filtering client-side.",
                parameters: [{ name: "id", in: "path", required: true, schema: { type: "string" }, description: "sub_<uuid> or sub_<number>; the sub_ prefix is optional." }],
                responses: {
                    "200": { description: "OK", content: { "application/json": { schema: { $ref: "#/components/schemas/Subscription" } } } },
                    "400": { description: "id is in neither supported form", content: { "application/json": { schema: { $ref: "#/components/schemas/Error" } } } },
                    "403": { description: "On-chain subscription belongs to another merchant wallet", content: { "application/json": { schema: { $ref: "#/components/schemas/Error" } } } },
                    "404": { description: "Not found for this merchant", content: { "application/json": { schema: { $ref: "#/components/schemas/Error" } } } },
                },
            },
            delete: {
                summary: "Cancel a subscription",
                description: "Path form of `DELETE /api/v1/subscriptions?id=`, with identical semantics: `sub_<uuid>` withdraws an unaccepted checkout, `sub_<number>` schedules cancel-at-period-end on an active subscription.",
                parameters: [{ name: "id", in: "path", required: true, schema: { type: "string" } }],
                responses: {
                    "200": { description: "Canceled or cancellation scheduled", content: { "application/json": { schema: { $ref: "#/components/schemas/Subscription" } } } },
                    "404": { description: "Not found", content: { "application/json": { schema: { $ref: "#/components/schemas/Error" } } } },
                    "409": { description: "This checkout can no longer be canceled", content: { "application/json": { schema: { $ref: "#/components/schemas/Error" } } } },
                },
            },
        },
