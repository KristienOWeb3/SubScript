...
* **Enterprise Merchant (`ENTERPRISE`)**:
  ...
  - Self-serve merchant signup is **open today**: the Enterprise Merchant option is selectable on `/signup`, and `/signup?role=merchant` funnels straight into email/Google onboarding.
  - **Invite-only merchant signup (mainnet).** Admin → Merchant Access holds a switch that closes public merchant signup. It is **off**, and stays off until mainnet. With it on, only an email an admin has granted can open a merchant account; every other signup — anywhere, any method — becomes a `USER` account. Flipping it is root-admin only (`ADMIN_WALLET_ADDRESSES`), while granting and revoking access is open to any admin.
    - Businesses ask at `/merchant-access` (or by DM to [@SubScript_onarc](https://x.com/SubScript_onarc)). Requests land in Admin → Merchant Access.
    - Approving writes a row to `merchant_access_grants`, keyed by email, and emails an invite link (`/signup?role=merchant&invite=<token>`). **The email is the authority, not the link:** `/api/auth/register-role` re-checks the grant against the wallet's verified email, so a forwarded invite opens nothing. Links can be re-copied or re-issued from the console; revoking kills the link but leaves an already-opened merchant account running.
    - A grant is refused for an email that already has a SubScript account — a `USER` account is never upgraded to `ENTERPRISE`, so that grant could never be redeemed. The business needs a different business email.
    - `ALLOW_PUBLIC_MERCHANT_SIGNUP="false"` (accepts `false`/`0`/`no`/`off`) forces enforcement on without a database read, as a break-glass. Under enforcement the legacy shared `MERCHANT_SIGNUP_CODE` is **ignored** — one code that anyone can forward is exactly what per-business grants replace. It still works as the old soft gate while enforcement is off.
