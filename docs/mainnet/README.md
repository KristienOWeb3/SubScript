...
| 2026-09-09 | Production Standby & Vercel Limit Safeguard | Activated production standby mode on `src/proxy.ts`: all live production page requests serve static black screen with white text 'SubScript will be back on mainnet' using Sukar font (cached at edge with `s-maxage=86400` to eliminate compute), and all `/api/*` endpoints return 503. Localhost remains completely unrestricted so local development and mainnet preparation continue smoothly. Cleared Vercel crons in `vercel.json` and commented out scheduled triggers in `.github/workflows/keepers.yml` and `sponsor-health.yml`. | Antigravity AI |
