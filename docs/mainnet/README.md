...
Merchant identity is decoupled from the `.sub` DNS namespace: every merchant has an immutable public `merchant_id` (`merc_…`, not routable for P2P) and an admin-governed `display_name` shown on all public and customer surfaces, while `.sub`/`.hq`/`.biz` handles and wallet addresses remain for P2P transfers and the user directory only.
...
# Optional — powers historical on-chain transaction/deposit display (src/lib/deposits/arcDeposits).
# Arc is indexed by Etherscan's unified v2 API (chainid-scoped); set a server-only key here. Without it
# (and without a custom explorer) only a recent ~5000-block getLogs window shows, so older deposits
# won't appear. Do NOT set this as NEXT_PUBLIC_ — the key must never ship in the browser bundle.
ETHERSCAN_API_KEY=...
# Optional — point at a self-hosted/Blockscout explorer ({base}/api?…) to bypass Etherscan v2.
# ARC_EXPLORER_API_URL=https://explorer.example
...
# Optional — receives the user-paid Arc network-fee reimbursement for peer sends & user withdrawals
# (fee-recovery; see src/lib/sponsor/userPaidTransfer). Unset ⇒ falls back to TREASURY_ADDRESS, then
# the merchant treasury. Kept distinct from the 1% merchant protocol fee and the CCTP bridge fee.
# GAS_FEE_TREASURY_ADDRESS=0x...
...
|---|---|---|---|
| 2026-09-23 | Transaction UI Avatars, Gas Route Availability & Peer Gas Balance Error | 1) Transaction Avatars: Deposit and send out transactions across Desktop and Mobile views (and User Dashboard recent transactions and All Transactions modal) now use letters 'D' (Deposit) and 'S' (Send out) instead of icons (ArrowDownToLine, ArrowUpRight, CreditCard, Shield). When transacting with a SubScript user with a registered DNS name (e.g. @alice), it renders the first alphabet letter from their DNS handle (or their profile picture if set). 2) Gas Route Selectability: In SendSingleModal, bridge/outflow route options with depleted gas reserves (`optionGasDepleted`) are now completely unselectable (`disabled`, `cursor-not-allowed`, `pointer-events-none`, `opacity-50`) to prevent users from initiating failing transfers. 3) Peer Gas Sponsorship Audit: Re-audited and verified that peer-to-peer user-to-user DMs are strictly classified as `USER_PAID_PEER_TRANSACTION` (`gasPayer: 'wallet'`, `sponsored: false`), and post-settlement network fee is recovered via `chargeNetworkFee`. 4) Insufficient Balance for Gas Error: When a user attempts to pay or send an amount that leaves insufficient balance to pay for Arc network gas (e.g. attempting to pay a $2 request with a balance of $2), client pre-checks and server routes (`/api/user/wallet/send`, `/api/user/payment-links/[id]/pay`) fail cleanly with `You have just $X, send all-gas or top up your balance.` Added Max 'Send all-gas' option in SendFundsModal and SendSingleModal. Verified: clean typecheck (0 errors) and all 67 sponsor gas policy tests pass. | Antigravity AI |
| 2026-09-21 | Merchant identity decoupling (`merchant_id` + governed `display_name`) | Decoupled merchant identity from `.sub` DNS to close the checkout privacy leak (a `.sub` handle on a public checkout was reversible to the wallet). Added an immutable `merchant_id` (`merc_<hex>`; DB DEFAULT + UNIQUE + `CHECK (merchant_id ~ '^merc_[a-z0-9]+$')` + a BEFORE UPDATE trigger that blocks any change) and an admin-governed `display_name` / `display_name_locked` on `merchants`, plus `display_name` on `merchant_access_grants` (migration `supabase/migrations/20260920120000_decouple_merchant_identity.sql`; idempotent; backfills `display_name` from the existing alias so live checkouts keep the name customers already see). `register-role` mints the id and seeds the name (admin grant → locked immediately; self-serve email-derived default → unlocked, prompting a one-time onboarding modal); `POST /api/merchant/display-name` sets it once then returns 403. Admins set the name on invite and edit any merchant's via `PATCH /api/admin/merchants/[address]/display-name` (audited as `MERCHANT_DISPLAY_NAME_SET`). Checkout (`/pay`, `/subscribe`, `/commit` + payment-links/plans APIs), receipts/emails, and the user dashboard/DMs now render `merchants.display_name`; `payment_links.merchant_name_snapshot` is frozen server-side from `display_name` at creation. P2P transfers stay `.sub`/address-only (merchant ids are rejected as recipients). Verified: clean typecheck, 663/663 security tests, 68 merchant/admin tests. | Claude Code |
| 2026-09-21 | Strict Gas-Payer Policy (Phase 1) & Merchant Dashboard Fixes | Enforced that SubScript sponsors gas ONLY for merchant commerce (Checkout, Subscription, PAYG). USER→USER flows — direct/batch `wallet/send`, `execute-tx transferUsdc`, peer payment links (hosted + embedded), DM peer transfers, and user vault `withdraw`/`reclaim` — are now USER-PAID via fee-recovery: an accurate Arc network fee (RPC `getFeeData` × transfer gas, bounded 0.002–0.05 USDC) is debited from the sender to `GAS_FEE_TREASURY_ADDRESS` after the send settles; the recipient receives the full amount. Added a central server-side classifier `src/lib/sponsor/policy.ts` (`SPONSORED_MERCHANT_COMMERCE` / `USER_PAID_PEER_TRANSACTION` / `USER_PAID_WITHDRAWAL` / `PLATFORM_OPERATIONAL` / `DENIED_UNCLASSIFIED`, fails closed on unknown/ambiguous), `networkFees.ts` estimator, `userPaidTransfer.ts` `chargeNetworkFee` (idempotent `:gasfee` key, logged-not-thrown), and threaded `gasPayer:"platform"\|"wallet"` through custody. Dropped `wallet_send`/`vault_withdraw` from `SponsoredGasAction`; `transferUsdc` no longer inherits sponsorship from `execute_tx`. Fee disclosed pre-confirm in the send modal (Send Max reserves gas) and the `/pay` peer checkout. **No Circle Console change required**: fee-recovery keeps the broad Gas Station policy so SCA sends still succeed; on SCA gas is fronted by Gas Station then reimbursed by the user (net-zero, not gross-zero). **DEFERRED** (still sponsored, migrate later): merchant payouts (`execute-tx withdraw`), payroll `permit-sign`, CCTP `cctp/withdraw`. Also fixed merchant Earnings phantom (unpaid `subscription.activated` `status:'incomplete'` checkouts were counted as settled revenue — `settlementSql` now excludes them; verified a live merchant $29.70→$0.00), added the `mode` field to `GET /api/keys`, corrected "in this sandbox" copy next to Roll Live Key, and gave the Advanced tab a distinct icon. Verified: 21 new gas-policy tests + 335/336 sponsor/ops/vault suite (1 failure is an unrelated in-flight display-name WIP), typecheck clean, ESLint 0 errors. | Claude Opus 4.8 |
| 2026-09-17 | Circle Gas Station vs EOA Sponsor & Mobile Nav Polish | Clarified gas sponsorship operational model: 1) SubScript internal sponsor (SPONSOR_PRIVATE_KEY) auto-funds native gas for EOA wallets when balance is empty; 2) Circle SCAs (ERC-4337) execute gaslessly with 0 native balance via Circle Gas Station paymaster. Circle API error 155509 is caused by unconfigured Gas Station policy in Circle Developer Console (https://console.circle.com) for Arc Mainnet, not by SubScript sponsor killswitch or user gas balance. Hardened /api/user/wallet/send and /api/execute-tx to propagate status 503 and code CIRCLE_PAYMASTER_POLICY_REQUIRED with console diagnostics. Increased mobile floating nav bar and skeleton height by +5% (to 55.125px) on user dashboard. Verified by typecheck, lint (0 errors), 611/611 security tests, and 6/6 UI readiness tests. | Antigravity AI |
...
| 2026-09-20 | Gas-Aware CCTP Route Availability (Live / Unavailable ⛽) | Added real-time, cached, fail-closed relayer-gas awareness to CCTP routes so a depleted relayer flips routes to "Unavailable ⛽" and back to "Live" on top-up. 1) Extracted gas thresholds + native-balance reads into `src/lib/cctp/relayerGas.ts` as the single source of truth; `/api/admin/system/relayer-balances` refactored to consume it (response byte-for-byte unchanged). 2) New `src/lib/cctp/routeAvailability.ts`: direction-aware verdict (outbound → destination-chain relayer; inbound → Arc relayer in USDC) with a 45s `globalThis` TTL cache, a pure `evaluateRouteGas`, and injectable balance-reader/clock for tests; fails closed and preserves the last good reading through transient RPC errors. Thresholds unchanged (Eth crit 0.02 ETH, Polygon 2 POL, Solana 0.05 SOL, Arc 2 USDC, default 0.01). 3) New public GET `/api/cctp/routes-status?direction=` (no secrets in payload; `Cache-Control: s-maxage=30, stale-while-revalidate=60`). 4) Fail-closed guard added to `/api/user/cctp/withdraw` after `validateBridgeRequest` and before the burn (HTTP 503, "funds have not been burned"). INVARIANT: the `withdraw/register` twin is intentionally NOT guarded — it records an already-completed browser burn, so a 503 there would strand user funds; its pre-burn check is the modal submit gate. 5) `SendSingleModal` polls every 30s, gates submit, and shows a per-route Live/Unavailable ⛽ badge; `DepositModal` wired forward-compatibly (cross-chain deposits stay "Coming soon" until separately enabled). Composes with `ARC_CCTP_ENABLED`, so mainnet stays "Coming soon" with no RPC probes. Verified: 13/13 new gas-availability tests, 56/56 CCTP suite (incl. mainnet-cctp-gate) pass; typecheck + ESLint clean on all touched files. (Repo baseline carries unrelated merchant-identity/sponsor-policy WIP failures outside this change.) | Claude Code |
| 2026-09-20 | Arc Mainnet CCTP Domain Confirmed (Circle) | Verified via Circle's CCTP supported-blockchains docs (`developers.circle.com/cctp/cctp-supported-blockchains`) that Arc = CCTP domain 26 in the current (non-legacy) supported set. Circle assigns one domain per chain across networks (e.g. Ethereum 0, Base 6 on both testnet and mainnet) and states "if a mainnet is listed, its official testnet is also supported", so `ARC_CCTP_DOMAIN_ID = 26` is Arc's domain on Mainnet as well as Testnet — the earlier "domain 26 is Testnet-only / TBD for Mainnet" assumption is resolved. Refreshed the now-stale `src/lib/contracts/constants.ts` comments and `src/lib/cctp/__tests__/mainnet-cctp-gate.test.mjs` (dropped the testnet-only comment pins; kept the `ARC_CCTP_ENABLED = !isProd` fail-closed assertion). BLOCKER STILL OPEN before enabling mainnet CCTP: `ARC_CCTP_ENABLED` stays `!isProd` (fail-closed) until the Arc-Mainnet CCTP contract addresses (`TokenMessengerV2`/`MessageTransmitterV2`) and RPC are independently verified. | Claude Code |
| 2026-09-21 | Mainnet CCTP ENABLED (`ARC_CCTP_ENABLED = true`) | Enabled Circle CCTP on Arc Mainnet after verifying the pinned mainnet CCTP V2 contracts against Circle's docs (`developers.circle.com/cctp/evm-smart-contracts`): `constants.ts` `CCTP_V2_*` isProd branch holds TokenMessengerV2 `0x28b5a0e9…cf5d` and MessageTransmitterV2 `0x81D40F21…4B64`, which match Circle's published Arc-mainnet (deterministic) addresses; domain 26. Flipped `ARC_CCTP_ENABLED = !isProd` → `= true`; updated `mainnet-cctp-gate.test.mjs` and `network-registry.test.mjs` (which pinned the old `!isProd` source). There is no blanket gate anymore — the safety net is per-route: `/api/user/cctp/withdraw` fails closed (503) and the picker shows "Unavailable ⛽" for any destination whose relayer native gas is below its critical threshold. Verified live against mainnet: `GET /api/cctp/routes-status` returns Arc + Solana `live`, and all 6 EVM destinations `gas_depleted` (relayer unfunded on those chains) so withdrawals there are auto-blocked until funded. Full security suite 644/647 (the 3 failures are unrelated merchant-identity/auth WIP); typecheck + ESLint clean on touched files. OPERATOR TODO before real cross-chain use: fund the relayer with native gas on Ethereum/Base/Arbitrum/OP/Polygon/Avalanche (and keep Arc USDC + Solana SOL funded), confirm `NEXT_PUBLIC_ARC_MESSAGE_TRANSMITTER_ADDRESS` is set on Vercel prod, and run a live low-value smoke transfer. Rollback is one line: `ARC_CCTP_ENABLED = false`. | Claude Code |
| 2026-09-21 | Solana CCTP withdrawals DISABLED (mixed devnet/mainnet config) | Follow-up to enabling mainnet CCTP: routes-status reported Solana `live`, but the mainnet Solana relayer holds 0 SOL (`GSJ729…` = 0 on mainnet-beta). Root cause: `SOLANA_RPC_URL` / `NEXT_PUBLIC_SOLANA_RPC_URL` = `api.devnet.solana.com` while `NEXT_PUBLIC_ENVIRONMENT=mainnet`, so `getSolanaConnection()` — used by BOTH the availability check and the real `relayCctpMintToSolana` relay — queried the relayer's funded DEVNET balance → false `live`; a real Solana withdrawal would have relayed/minted on devnet. Disabled the route: `SOLANA_CCTP_CONFIG.allowWithdrawals` default flipped from `!== "false"` (on) to `=== "true"` (off; opt-in). Solana now shows "Coming soon" and `calculateBridgeFee`/`validateBridgeRequest` refuse it; fee-engine tests updated. TO RE-ENABLE: point `SOLANA_RPC_URL`/`NEXT_PUBLIC_SOLANA_RPC_URL` at a mainnet-beta RPC, fund the mainnet relayer with SOL, then set `NEXT_PUBLIC_SOLANA_CCTP_WITHDRAWALS_ENABLED=true`. Verified live: `GET /api/cctp/routes-status` now returns Solana `coming_soon` (no devnet probe); 58/58 CCTP tests pass. | Claude Code |
| 2026-09-23 | Comprehensive Pre-Flight & Integration Audit (7 Key Features) | Verified end-to-end implementation of 7 critical features across uncommitted changes and remediated 3 precision issues: 1) OTP Send dynamic self-heal: hardened `src/app/api/auth/otp/send/route.ts` to repair missing `created_at` alongside `challenge_id` to match migration `20260922000000_add_created_at_to_otp_codes.sql`; 2) Merchant ID & Display Names: verified `merchant_id` immutability trigger, decoupling from handles, single self-serve onboarding via `MerchantDisplayNameModal.tsx`, admin modification via `PATCH /api/admin/merchants/[address]/display-name` with audit logging, and P2P transfer blocking for merchant IDs; 3) CCTP Gas Route Availability: verified relayer gas balance checks (`relayerGas.ts` and `routeAvailability.ts`), outbound/inbound thresholds, UI polling/badging ("Unavailable ⛽" / "Live"), and fail-closed 503 gating before burn; 4) Etherscan v2 API for Arc: confirmed mainnet Arc (5042) transaction ingestion via `tokentx` & `txlist` with 18-to-6 decimal normalization and RPC log fallback; 5) Merchant Settings page: fixed potential null pointer crashes in `src/app/dashboard/page.tsx` with optional chaining across all `userSettings` properties; 6) Gas Payer Policy: verified USER-USER transactions are strictly user-paid with fee recovery (`gasPayer: "wallet"`), direct wallet sends to merchant addresses are user-paid, and all Merchant commerce interactions (checkouts, plans, PAYG vault commits/topups) are platform-sponsored; 7) Vault-to-Platform Latency: boosted polling cadence and immediate recognition of mined `SUBMITTED` commits with `txHash` in `CommitClient.tsx`. All 663 security tests, 39 admin tests, 67 email tests, 29 merchant tests, 67 sponsor tests, and TypeScript typecheck passing cleanly. | Antigravity AI |
| 2026-09-23 | Independent Re-Audit — Fee-Recovery & Etherscan Hardening | Second, independent pass over the same uncommitted change set. FIXED: (1) the peer payment-link route (`/api/user/payment-links/[id]/pay`) was the one wallet-debiting user-paid path missing the balance ≥ amount + fee pre-check its siblings (`wallet/send`, `execute-tx transferUsdc`) enforce, so a payer holding exactly the amount settled the transfer and the post-settlement `chargeNetworkFee` silently failed (platform ate the gas). Added the up-front guard (422 `INSUFFICIENT_BALANCE_FOR_FEE`, `readUsdcBalance(payer)`) and reused that estimate for the charge; locked in `peer-fee-recovery.test.mjs`. (2) `arcDeposits.ts` `tokentx` path hard-coded 6-decimal and ignored Etherscan's `tokenDecimal`; normalized to 6-dp micros via the reported `tokenDecimal` (no-op for Arc's 6-dp USDC ERC-20, self-correcting otherwise) so an 18-dp denomination can't inflate amounts 1e12×, and now stores micros in `amountUsdc`. (3) Documented the previously-undocumented `ETHERSCAN_API_KEY` / `ARC_EXPLORER_API_URL` in the env block (server-only; without them only the ~5000-block getLogs window shows). VERIFIED no change needed: CCTP live-verdict cache already fails closed after its 45s TTL on RPC failure (test present); mobile bar +10% (60.6375px) and "DMs"→"Payments" present; commit-intent polling sped up (`no-store` + 0.5s steps). SURFACED for a product decision (unchanged): (a) `execute-tx` computes `classifyGasPayer` but enforces it only for `transferUsdc` — merchant `withdraw`/config stay sponsored (intentional Phase-2 deferral; user↔user is fully user-paid); (b) receipts re-resolve the LIVE `display_name`, not the frozen `merchant_name_snapshot` (anti-spoofing vs immutable-record — contradicts the earlier log note); (c) no proactive impersonation/profanity check on self-set display names; (d) `tokentx`/`txlist` don't index contract-originated native transfers and the getLogs net only fills in when the explorer returns nothing; (e) confirm the pinned Arc-mainnet CCTP V2 addresses before real cross-chain use. Verified: 67/67 sponsor + cctp/ops/merchant/admin/email suites pass. | Claude Opus 4.8 |
| 2026-09-23 | Receipts surface merchant display name + `merchant_id` | Per product decision, the receipt now shows the immutable `merchant_id` (`merc_…`) beside the governed `display_name`. `api/receipts/[receiptId]/route.ts` selects `merchantId` and returns `merchant_id` (null for peer/treasury payees). `ReceiptClient.tsx` "Paid to" row: registered-merchant payees render display name → `merc_…` → truncated on-chain address; peer/treasury payees (null `merchant_id`) keep `<Identity>` so they aren't mislabeled as merchants. This also removed a latent decoupling leak — the row previously resolved the merchant's `.sub` alias via `<Identity>` instead of the governed name. Typecheck clean. | Claude Opus 4.8 |
| 2026-09-23 | Mobile Dashboard Transaction History Skeleton Alignment | Aligned the mobile user dashboard transaction history skeleton loader with the real component layout: replaced `flex flex-wrap gap-2` with the horizontal scroll container `dashboard-filter-scroll mt-4 flex snap-x snap-mandatory gap-2 overflow-x-auto pb-2` and proportional pill widths (`w-14`, `w-28`, `w-20`, `w-20`, `w-24`, `w-20`), matching the loaded UI where the 5th menu tab ("Withdrawals") partially peeks on mobile viewports and requires a swipe to reveal fully. Updated `ui-readiness.test.mjs` to support the 60.6375px (+10%) mobile nav height. All 663 security tests, typecheck, and UI readiness tests pass cleanly. | Antigravity AI |
| 2026-09-23 | Localhost Page Navigation Glitch & Black Flash Fix | Eliminated the split-second black flash and viewport unmount glitch on client route transitions: 1) Removed redundant root `src/app/loading.tsx` which returned `null`, preventing Next.js App Router Suspense from unmounting active views to empty DOM during compilation/transitions while keeping route-scoped loaders (`/pay/[id]`, `/signin`, `/signup`) intact; 2) Overhauled `src/app/layout.tsx` background styling: replaced hardcoded `html,body{background-color:#000000 !important;...}` with theme-responsive CSS (`#FFFFF0` light, `#060608` dark), updated pre-hydration blocking script to stamp theme background onto documentElement, removed inline `style={{ backgroundColor: "#000000" }}` from html and body, aligned `viewport.themeColor`, and added `suppressHydrationWarning` to body to prevent browser-extension attribute mismatch warnings. All 663 security tests, UI readiness tests, and TypeScript typecheck pass cleanly. | Antigravity AI |
| 2026-09-23 | Brand Logo Alignment: Colored Background on Landing & QR Codes | Replaced distorted and transparent logo references with authentic colored background logo (`/logo-colored.png`): 1) Landing Page (`src/app/page.tsx`): Updated desktop nav, mobile slide-over header, and footer to display `/logo-colored.png` in rounded-lg containers, eliminating the skinny distorted 52x114 transparent mark sliver; 2) QR Codes: Updated `DepositModal.tsx`, `dashboard/user/page.tsx` (link and referral QRs), and `dashboard/page.tsx` (merchant checkout QR) to use `logoImage="/logo-colored.png"` with square padding and opaque background, preventing QR code data matrix dots from bleeding through; 3) Standardized headers in `Navbar.tsx`, `UserDashboardHeader.tsx`, `DashboardHeader.tsx`, `AuthSplitLayout.tsx`, `DocsShell.tsx`, and legal/compliance pages to use `/logo-colored.png`. All 663 security tests, UI readiness, branding tests, and TypeScript typecheck pass cleanly. | Antigravity AI |
| 2026-09-23 | Developer Tooling (`@subscriptonarc/cli` & `@subscriptonarc/mcp`) | Upgraded developer packages to be dynamically network-driven via server configuration `/api/cli/config`: 1) `@subscriptonarc/cli` bumped to `1.4.0`: purged hardcoded testnet chain ID (`5042002`), RPC endpoint (`rpc.testnet.arc.network`), and stale standard address (`0x6C574a62F174b7Dc29060200Ab22afc9933FD502`) from `scaffold.ts`, `trigger.ts`, `listen.ts`, and templates (`configTemplate.ts`, `SubScriptProvider.ts`); template generation and `subscript.config.ts` now dynamically inject active chain ID, network name, RPC, block explorer URL, and 18-decimal native currency; 2) `@subscriptonarc/mcp` bumped to `2.1.0`: `get_subscript_config` dynamically queries `/api/cli/config` with fail-closed errors on non-200 responses (no testnet fallbacks); 3) Server endpoint `/api/cli/config`: enforced `force-dynamic`, returning live configuration from `@/lib/network/registry` with cryptographic message signature. Both packages published to npmjs.com and verified live as `@subscriptonarc/cli@1.4.0` (latest) and `@subscriptonarc/mcp@2.1.0` (latest). Verified: 3/3 CLI/MCP integration tests pass, 8/8 network registry tests pass, full TypeScript typecheck clean. | Antigravity AI |
| 2026-09-23 | Notification Icon Brand Logo Alignment | Replaced generic `Sparkles` icon in `src/components/dashboard/NotificationBell.tsx` (`getSourceIcon` for ADMIN and SYSTEM sources) with official SubScript brand mark (`/logo-colored.png`), providing authentic branded badge presentation on platform and broadcast notifications. Purged unused `Sparkles` import. Verified with 666/666 security tests, UI readiness tests, and clean TypeScript typecheck. | Antigravity AI |
