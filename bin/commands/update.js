...
export async function runUpdate(options) {
    ...
    console.log("==================================================");
    await sendTelemetry("cli.update.started", { cliVersion: CLI_VERSION, templateVersion: TEMPLATE_VERSION, requestId }, noTelemetry);
    try {
        ...
        const updateFile = async (filePath, fileName, newContent) => {
        ...
        await sendTelemetry("cli.update.completed", { merchantId: merchantAddress, cliVersion: CLI_VERSION, templateVersion: TEMPLATE_VERSION, mode, requestId }, noTelemetry);
    }
    catch (err) {
        console.error(`\n[ERROR] Update failed: ${err.message}`);
        await sendTelemetry("cli.update.failed", { cliVersion: CLI_VERSION, templateVersion: TEMPLATE_VERSION, requestId }, noTelemetry);
        process.exit(1);
    }
}
