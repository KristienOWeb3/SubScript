...
function isVersionOutdated(current, minSupported) {
...
export async function runVerify(options) {
    ...
    let status = "PASS";
    const logs = [];
    await sendTelemetry("cli.verify.started", { cliVersion: CLI_VERSION, templateVersion: TEMPLATE_VERSION, requestId }, noTelemetry);
    try {
        // 1. Framework Check
                ...
                if (!merchant || !mode || !version) {
                    ...
                    try {
                        ...
                        if (isVersionOutdated(version, minSupported)) {
                            status = "WARNING";
                            logs.push(`[!] Version Compatibility: WARNING. Your integration is outdated.\n` +
        ...
        if (paths.hasBackend && existsSync(paths.webhookPath)) {
            ...
            if (!secretFound) {
                status = "FAIL";
                logs.push("[-] Webhook Configuration: FAILED. Webhook route is scaffolded, but SUBSCRIPT_WEBHOOK_SECRET " +
        ...
        console.log("==================================================");
        await sendTelemetry("cli.verify.completed", { cliVersion: CLI_VERSION, templateVersion: TEMPLATE_VERSION, requestId }, noTelemetry);
    }
    catch (err) {
        console.error(`\n[ERROR] Verification aborted: ${err.message}`);
        await sendTelemetry("cli.verify.failed", { cliVersion: CLI_VERSION, templateVersion: TEMPLATE_VERSION, requestId }, noTelemetry);
        process.exit(1);
    }
}
