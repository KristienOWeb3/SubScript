...
function installDeps(cwd, pm, packages) {
    const pkgString = packages.join(" ");
    let cmd = `npm install ${pkgString}`;
    if (pm === "pnpm")
        cmd = `pnpm add ${pkgString}`;
    else if (pm === "yarn")
        cmd = `yarn add ${pkgString}`;
    else if (pm === "bun")
        cmd = `bun add ${pkgString}`;
...
export async function runInit(options) {
    ...
    await sendTelemetry("cli.init.started", { cliVersion: CLI_VERSION, templateVersion: TEMPLATE_VERSION, requestId }, noTelemetry);
    try {
        // 1. Framework detection (Phase 3 / Addition 7)
        const framework = await detectFramework(cwd);
        console.log(`[INFO] Detected framework: ${framework}`);
        if (framework === "unsupported") {
            throw new Error("Unsupported framework structure. SubScript CLI supports React-based projects (Next.js or React SPA) with a package.json file.");
        }
        // 2. Resolve parameters via Dashboard Onboarding Bridge
        const sessionToken = options.session;
        if (!sessionToken) {
            throw new Error("Missing required flag: --session <TOKEN>.\n" +
        ...
        // Telemetry completed
        await sendTelemetry("cli.init.completed", {
        ...
        // Telemetry failed
        await sendTelemetry("cli.init.failed", { cliVersion: CLI_VERSION, templateVersion: TEMPLATE_VERSION, requestId }, noTelemetry);
        process.exit(1);
    }
}
