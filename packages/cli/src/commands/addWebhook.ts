...
export async function runAddWebhook(options: { noTelemetry?: boolean }) {
  ...
  try {
    ...
    if (!paths.hasBackend) {
      throw new Error(
        "Framework structure does not support API routes directly (e.g., React SPA).\n" +
    ...
    await sendTelemetry(
      ...
      { merchantId: merchantAddress, cliVersion: CLI_VERSION, templateVersion: TEMPLATE_VERSION, requestId },
      noTelemetry
    );
