...
1. THE REST TRUTH: SubScript integrates over a plain REST API — there is no SDK package. Do not
   install or import \`@subscriptonarc/sdk\`; it does not exist.
...
Commands:
  ...
  add checkout          Scaffold the checkout intent server route + button.
  add webhook           Scaffold the signed webhook receiver route.
...
async function main() {
  ...
  switch (args.command) {
    case undefined:
    case "init":
      /* A dashboard session token enables the non-interactive bridge; otherwise run the
         self-contained interactive wizard so first-time users need nothing but the CLI. */
      if (args.session) {
    ...
    case "wizard":
      ...
      break;
    case "add":
      if (args.sub === "checkout") await runAddCheckout({ noTelemetry: args.noTelemetry });
      else if (args.sub === "webhook") await runAddWebhook({ noTelemetry: args.noTelemetry });
      else {
        console.error(`Unknown 'add' target: ${args.sub ?? "(none)"}. Try 'add checkout' or 'add webhook'.`);
        process.exit(1);
      }
      break;
    case "doctor":
