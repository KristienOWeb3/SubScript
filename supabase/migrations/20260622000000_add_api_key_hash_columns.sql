 ...
 *
 * Phase B (docs/runbooks/null_api_key_plaintext_after_hash_rollout.sql) nulls the plaintext, and
 * must be run manually only AFTER the hash-aware code is deployed and verified.
 */
...
/* Authentication lookups are by hash; secret hashes should be globally unique. */
CREATE UNIQUE INDEX IF NOT EXISTS api_keys_secret_hash_idx
    ON api_keys(secret_key_hash)
